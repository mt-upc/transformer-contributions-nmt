...
def get_word_word_attention(token_token_attention, src_word_to_bpe, trg_word_to_bpe, remove_EOS=True):
    ...
    for word in src_word_to_bpe:
        ...
        #word_word_attention[:, word[0]] = word_word_attention[:,word[0]]
    ...
    #print(word_word_attention)

    # mean the attention weights or vector norms for all tokens in a target word that has been split
    for word in trg_word_to_bpe:
...
def alignment_extract(seed, setting = "AWO"):
    ...
    with open('./work/results_seed' + str(seed) + '/head_importance/extracted_head_importance.pkl', 'rb') as handle:
        head_importance = pickle.load(handle)
    # with open('./work/results_seed' + str(seed) + '/head_importance/extracted_head_importance_input_out_proj.pkl', 'rb') as handle:
    #     head_importance = pickle.load(handle)

    for mode in ["attn", "summed_afx"]:
        for l in range(6):
            with open("./work/results_seed" + str(seed) + "/" + alignment_folder + "/whypothesis-{}-{}-{}_no_eos".format(mode,l,setting), "w") as f:
                for i in range(len(src_bpe_sents)):
                    ...
                    if mode == "summed_afx":
                        ...
                        head_importance_np = head_importance[i][str(seed)][:,l,:] # (tgt_len,layer,num_heads) -> (tgt_len,num_heads)
                        attention_matrix_heads = torch.squeeze(extract_matrix[i][mode][l]).detach().numpy() # (num_heads,tgt_len,src_len)
                        attention_matrix_weighted = np.zeros(shape=(len(en_list),len(src_list)))
                        for h in range (4):
                            attention_matrix_per_head = attention_matrix_heads[h,:,:]
                            zeros_mask = np.zeros(shape=(len(en_list),len(src_list)))
                            zeros_mask[:,-2:] = attention_matrix_per_head[:,-2:]
                            if setting == "AWI":
                                zeros_mask[-1,-2] = 0
                            elif setting == "AWO":
                                zeros_mask[-2,-2] = 0
                            attention_matrix_per_head = attention_matrix_per_head - zeros_mask

                            #print(attention_matrix_per_head)

                            attention_matrix_head = attention_matrix_per_head*head_importance_np[:,h][:, None]
                            attention_matrix_weighted += attention_matrix_head
                        attention_matrix = attention_matrix_weighted

                        ## If standard layer average
...
def layer_aer_calculate(seed, setting="AWO"):
    ...
    for mode in ["attn", "summed_afx"]:
        for l in range(6):
            ...
            with open("./work/results_seed" + str(seed) + "/" + alignment_folder + "/whypothesis-{}-{}-{}_no_eos".format(mode,l,setting)) as f:
                for line in f:
...
def head_aer_calculate(seed, setting="AWO"):
    ...
    results_dict = {}
    for metric in ["aer", "recall", "precision"]:
            ...
            for l in range(6):
                for h in range(4):
                    ...
                    if metric == "aer":
                        ...
                        results[ids[mode]][l][h]= recall
                    else:
                        results[ids[mode]][l][h]= precision
...
if __name__ == '__main__':
    ...
    for seed in seeds:
        ...
        for setting in ["AWO", "AWI"]:
            if setting == "AWO":
                print("--------------- Alignment with output (AWO) setting ---------------")
            else:
                print("--------------- Alignment with input (AWI) setting ---------------")
            
            alignment_extract(seed, setting=setting)
            layer_results = layer_aer_calculate(seed, setting) * 100
            head_results = head_aer_calculate(seed, setting)['aer'] * 100 
    ...
    for setting in ["AWO", "AWI"]:
        ...
        print("correlation with averaged norm:")
        print("\tSpearman's p:\t\t\t\t", list_mean(seed_average_spearman[setting]))
